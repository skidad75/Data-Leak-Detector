...
def generate_security_summary_pdf(url, emails, login_pages, console_pages, security_info, data_leaks, network_info):
    class PDF(FPDF):
        ...
        def multi_cell_with_wrap(self, w, h, txt, border=0, align='J', fill=False):
            ...
            # If the text is wider than the cell, wrap it
            if text_width > w:
                words = txt.split()
                line = ''
                for word in words:
                    if self.get_string_width(line + ' ' + word) <= w:
                        ...
                        self.cell(w, h, line, border, 1, align, fill)
                        line = word
                if line:
                    self.cell(w, h, line, border, 1, align, fill)
            else:
    ...
    for header, value in security_info['Security Headers'].items():
        pdf.multi_cell_with_wrap(0, 10, f"{header}: {value}", 0)
    pdf.ln(5)
    ...
    pdf.set_font('Arial', '', 12)
    for leak_type, leaks in data_leaks.items():
        pdf.multi_cell_with_wrap(0, 10, f"{leak_type}: {', '.join(list(leaks)[:5])}", 0)
    pdf.ln(5)
    ...
    pdf.set_font('Arial', '', 12)
    pdf.multi_cell_with_wrap(0, 10, f"IP Address: {network_info['IP Address']}", 0)
    pdf.multi_cell_with_wrap(0, 10, f"Open Ports: {', '.join(map(str, network_info['Open Ports']))}", 0)
    pdf.multi_cell_with_wrap(0, 10, f"Server Info: {network_info['Server Info']}", 0)
    pdf.ln(5)
    ...
    for rec in recommendations:
        pdf.multi_cell_with_wrap(0, 10, f"• {rec}", 0)
