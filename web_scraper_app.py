...
# Streamlit app
st.title("Web Scraper and Security Analyzer")

# User input
input_url = st.text_input("Enter the URL to analyze:")
max_depth = st.slider("Maximum depth for crawling:", 1, 5, 3)
...
if st.button("Scrape and Analyze"):
    if input_url:
        ...
        with col1:
            ...
            st.subheader("Potential Data Leaks (First 10 per type)")
            for leak_type, leaks in data_leaks.items():
        ...
        email_list = df_emails['Email'].tolist() if 'Email' in df_emails.columns else []
        
        # For login pages and console pages, use the first column if 'URL' is not present
        login_pages_list = (df_login_pages['URL'].tolist() if 'URL' in df_login_pages.columns 
                            else df_login_pages.iloc[:, 0].tolist() if not df_login_pages.empty else [])
        
        console_pages_list = (df_console_pages['URL'].tolist() if 'URL' in df_console_pages.columns 
                              else df_console_pages.iloc[:, 0].tolist() if not df_console_pages.empty else [])
